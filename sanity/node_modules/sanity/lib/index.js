...
var _internalBrowser = require('./_chunks/_internalBrowser-SikHWaiu.js');
...
const ChangeFieldWrapper = props => {
  ...
  const {
    onSetFocus
  } = React.useContext(_internalBrowser.ConnectorContext);
...
function PresenceScope(props) {
  ...
  const contextPresence = React.useContext(_internalBrowser.FormFieldPresenceContext);
...
const remoteSnapshots = _internalBrowser.memoize((client, idPair, typeName) => {
  return _internalBrowser.memoizedPair(client, idPair, typeName).pipe(operators.switchMap(_ref15 => {
    let {
      published,
      draft
    } = _ref15;
